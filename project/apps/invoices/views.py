...
from apps.customers.models import Customer, ProductSubscription, SubscriptionUsage
...
def invoice_list(request):
    ...
    if month_filter:
        try:
            ...
            invoices = invoices.filter(invoice_date__month=month_num)
    ...
    if year_filter:
        try:
            ...
            invoices = invoices.filter(invoice_date__year=year_num)
        except (ValueError, TypeError):
            pass
    
    # Date range filter (takes precedence over month/year if specified)
    if date_from:
    ...
    available_years = Invoice.objects.dates('invoice_date', 'year', order='DESC').values_list('invoice_date__year', flat=True)
    ...
    month_filter_display = ''
    if month_filter:
        month_dict = dict(month_choices)
        month_filter_display = month_dict.get(month_filter, '')
    ...
    context = {
        ...
        'month_filter_display': month_filter_display,
...
def invoice_create(request):
    ...
    # Set proper initial dates using relativedelta
    today = timezone.now().date()
    initial_data = {
        'invoice_date': today,
        'due_date': today + relativedelta(months=1)  # One month from today
    }
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            ...
            if hasattr(customer, 'has_active_subscriptions') and customer.has_active_subscriptions():
...
def invoice_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            pass  # Ignore if SubscriptionUsage doesn't exist
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({
                ...
                'is_subscriber': getattr(customer, 'is_subscriber', False)
            })
        except:
            # If ProductSubscription model doesn't exist, return empty data
            return JsonResponse({
                'subscriptions': [],
...
def get_product_data(request):
    """AJAX view to get product data for invoice lines including stock information"""
    if request.method != 'GET':
        return JsonResponse({'error': 'Method not allowed'}, status=405)
    ...
    try:
        ...
        data = {
            ...
            'unit': product.get_unit_display(),  # Get display value for unit
            ...
            'hazard_level': product.get_hazard_level_display(),
        ...
        if customer_id:
            try:
                ...
                try:
                    ...
                    pass  # Ignore if ProductSubscription doesn't exist
            except (ValueError, TypeError):
                pass
...
def invoice_detail(request, pk):
    ...
    # Get customer subscription information (with error handling)
    ...
    try:
        if getattr(invoice.customer, 'is_subscriber', False):
        ...
        # If subscription models don't exist, ignore
        pass
