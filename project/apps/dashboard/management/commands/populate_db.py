...
class Command(BaseCommand):
    ...
    def create_products(self, categories):
        ...
        for prod_data in products_data:
            product, created = Product.objects.get_or_create(
                code=prod_data["code"], defaults=prod_data
            )
    ...
    def create_customers(self):
        ...
        for cust_data in customers_data:
            customer, created = Customer.objects.get_or_create(
                name=cust_data["name"], defaults=cust_data
            )
    ...
    def create_suppliers(self):
        ...
        for supp_data in suppliers_data:
            supplier, created = Supplier.objects.get_or_create(
                name=supp_data["name"], defaults=supp_data
            )
    ...
    def create_product_subscriptions(self, customers, products, users):
        ...
        subscribing_customers = random.sample(
            customers, max(1, int(len(customers) * 0.6))
        )
        ...
        for customer in subscribing_customers:
            ...
            available_products = [
                p
                for p in products
                if p.category.name in ["Services", "Équipements"] or not p.is_service
            ]
            selected_products = random.sample(
                available_products, min(num_subscriptions, len(available_products))
            )
            ...
            for product in selected_products:
                ...
                fixed_payment = base_price * Decimal(
                    str(random.uniform(0.7, 1.2))
                )  # 70-120% of base price

                if product.is_service:
                    max_quantity = Decimal("1.00")  # Services usually 1 unit
                else:
                    max_quantity = Decimal(
                        str(random.uniform(5.0, 20.0))
                    )  # Physical products
                ...
                self.stdout.write(
                    f"  Created subscription: {customer.name} - {product.name} ({billing_cycle})"
                )
    ...
    def create_subscription_usage(self, subscription, user):
        ...
        months_back = {"monthly": 3, "quarterly": 6, "yearly": 12}.get(
            subscription.billing_cycle, 3
        )
        ...
        for month_offset in range(months_back):
            ...
            quantity_used = subscription.max_quantity_allowed * Decimal(
                str(usage_percent)
            )
    ...
    def create_stock_movements(self, products, users):
        ...
        for product in physical_products:
            ...
            self.stdout.write(
                f"  Created initial stock for: {product.name} - Quantity: {initial_quantity}"
            )
            ...
            for _ in range(random.randint(1, 3)):
                ...
                self.stdout.write(
                    f"  Created movement: {movement_type} {quantity} for {product.name}"
                )
            ...
            for i in range(random.randint(1, 3)):
                ...
                invoice = Invoice.objects.create(
                    ...
                    driver_name=random.choice(
                        ["", "Karim Benzema", "Ali Hassan", "Omar Fadel"]
                    ),
                    vehicle_registration=random.choice(
                        ["", "12345-16-01", "54321-16-02"]
                    ),
                ...
                if customer.discount_rate > 0:
                    invoice.discount_amount = invoice.subtotal_ht * (
                        customer.discount_rate / 100
                    )
                    invoice.save()
                ...
                if invoice.status == "paid":
                    ...
                    Payment.objects.create(
                        ...
                        notes=random.choice(
                            [
                                "",
                                "Paiement reçu avec remerciements",
                ...
                self.stdout.write(
                    f"  Created invoice: {invoice.invoice_number} for {customer.name} - {status}"
                )
